...
Public Class DataSourceSearcher
    ...
    Private Property SearchData As New BindingList(Of DataUnit)()

    Public Sub New(searchTerm As String, searchData As BindingSource)
        ...
        Me.SearchData.AddRange(searchData.DataSource)
    End Sub
    ...
    Public Function GetComputers() As List(Of Computer)
        ...
        If Me.SearchTerm.Trim().Equals("/") OrElse Me.SearchTerm.Trim().Equals("\") Then
            ...
            searchResults.AddRange(Me.SearchData.OfType(Of Computer)())
        Else
            ...
            If Regex.Matches(Me.SearchTerm, "^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$").Count = 1 Then
                ...
                Else
                    ...
                    Dim matchingComputer As Computer = Me.SearchData.OfType(Of Computer)().SingleOrDefault(Function(c) c.IPAddress = Me.SearchTerm)
            ...
            If Not String.IsNullOrWhiteSpace(Me.SearchTerm) Then
                ...
                Dim matchingComputer = Me.SearchData.OfType(Of Computer)().SingleOrDefault(Function(c) c.Value.ToUpper().Equals(Me.SearchTerm.ToUpper()))
                If matchingComputer IsNot Nothing Then
                    ...
                    Try
                        ...
                        If Not String.IsNullOrWhiteSpace(wmiComputerName) Then
                            ...
                            Else
                                ...
                                MsgBox(message, icon:=MessageBoxIcon.Error, caption:=String.Format("Requested: {0} Response: {1}", Me.SearchTerm, wmiComputerName))
                            End If
                        End If
                    Catch ex As Exception
                ...
                Else
                    ...
                    searchResults.AddRange(Me.SearchData.OfType(Of Computer)().Where(Function(c) _
    ...
    Public Function GetComputer() As Computer
        For Each computer As Computer In Me.SearchData.OfType(Of Computer)()
